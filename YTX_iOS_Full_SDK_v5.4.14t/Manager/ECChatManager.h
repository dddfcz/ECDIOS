...
/**
 ...
 @param completion 执行结果回调block
 */
-(void)downloadHDImageMessage:(ECMessage*)message progress:(id<ECProgressDelegate>)progress completion:(void(^)(ECError *error, ECMessage* message))completion;
...
/**
 ...
 @param completion 执行结果回调block
 */
-(void)queryMessageReadStatus:(ECMessage*)message completion:(void(^)(ECError *error, NSArray* readArray, NSArray* unreadArray)) completion EC_DEPRECATED_IOS(5.2.2, 5.2.2,"Use - queryMessageReadStatus:msgId:pageSize:pageNo:completion:");
...
/**
 ...
 @param agentId 坐席id
 @param osUnityAccount 统一服务号
 @param completion 执行结果回调
 */
- (void)sendEventMessage:(NSInteger)evtType userAccount:(NSString *)userAccount agentId:(NSString *)agentId osUnityAccount:(NSString *)osUnityAccount  completion:(void(^)(ECError *error, NSString *agentId))completion;

/**
 @brief 发送消息
 
 @param message 发送的消息
 @param message type类型   1用户  2坐席
 @param completion 执行结果回调
 */
- (NSString *)sendServiceMessage:(ECMessage *)message senderType:(NSInteger)type completion:(void(^)(ECError *error, ECMessage *message))completion;
