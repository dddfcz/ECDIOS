...
/**
 ...
 @param completion 执行结果回调block
 */
-(void)deleteInterphoneMeeting:(NSString *)meetingNumber completion:(void(^)(ECError *error, NSString *meetingNumber))completion;
...
/**
 ...
 @param members    成员
 @param voiceMode  1：无提示音有背景音；2：有提示音有背景音；3：无提示音无背景音；
 @param autoDelete  会议是否自动删除
 @param completion 执行结果回调block
 */
-(void)createInterphoneMeetingWithMembers:(NSArray *)members andVoiceMode:(NSInteger)voiceMode autoDelete:(BOOL)autoDelete completion:(void(^)(ECError* error, NSString* meetingNumber))completion;
