...
/**
 ...
 @param callType 电话类型
 @param called 电话号(加国际码)或者VoIP号码
 @param toThird 第三方对接字冠
 @return 本次电话的id
 */
- (NSString *)makeCallWithType:(CallType)callType andCalled:(NSString *)called andToThird:(NSString *)toThird;
...
/**
 @brief 添加用户自定义VOIP数据。
 @param type : 类型
 enum {//appendUserData类型
 USERDATA_FOR_TOKEN    =0,
 USERDATA_FOR_USER_AGENT,
 USERDATA_FOR_INVITE//发起呼叫};
 @param data : 具体字段。例如"tel=13812345678;nickname=小云;confpwd=123"
 @return 0:成功  非0:失败
 */
- (NSInteger)appendUserData:(int)type data:(NSString *)data;
